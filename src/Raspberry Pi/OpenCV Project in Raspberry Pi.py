...
client = socket.socket()  # anounce the kind of socket，and generate the connection object at the same time
client.connect(('192.168.123.26', 50007))  # start connection   ip:local ip    port:6961
...
masktarget = cv2.imread("/home/pi/Desktop/target.jpg")  # get the area of rectangle including the holder
...
def StopTarget(image):
    ...
    mask = cv2.inRange(imgHSVtarget, lower, upper)  # get the threshold of green leaves
    kernel = np.ones((3, 3), np.uint8)
    mask = cv2.dilate(mask, kernel, iterations=2)  # dilate
...
while True:
    ...
    if StopTarget(imgHSV) and StopStems(imgHSV, 3):
        ...
        msg = ''.join(message)
        print(msg)
        client.send(msg.encode("utf-8"))  # deliver the message to the server
    ...
    if x1 == 22:
        cv2.waitKey(1500)
        data = client.recv(1024)  # accept the message
        print('client_recv:', data.decode())  # print the message
        break

    GPIO.cleanup()

client.close()  # Finally,remember to shut up the client
